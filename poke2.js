...
function displayError(message) { // Function to display an error message 
  ...
  pokemonInfo.innerHTML = `<p class="text-danger">${message}</p>`; // Set the inner HTML to show the error message styled with Bootstrap to dispaly in red.
}


document.getElementById('search-button').addEventListener('click', () => { // Attach a click event listener to the search button
  const input = document.getElementById('pokemon-input').value.trim(); // Remove spaces from leading and trailing from the input field
  if (input) { 
    fetchPokemonData(input); // If the input is not empty, fetch the Pokemon data
  } else {
